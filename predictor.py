...
class BasePredictor:
    ...
    def write_results(self, i: int, p: Path, im: torch.Tensor, s: List[str], timestamp: str) -> str:
        ...
        if self.args.save_txt:
            ...
            orig_img = self.results[i].orig_img
            ...
            for idx, box in enumerate(boxes.data):
                ...
                if cls == player_class:
                    ...
                    x_end = min(img_w, cx + crop_w // 2)
                    y_end = min(img_h, cy + crop_h // 2)

                    crop = orig_img[y_start:y_end, x_start:x_end]

                    crop_resized = cv2.resize(crop, (20, 20))
                    crop_flatten = crop_resized.reshape(-1)  # (20*20*3,)
                    player_features.append(crop_flatten)
            ...
            if len(player_features) >= 2:
                player_features_np = np.array([feat.cpu().numpy() if isinstance(feat, torch.Tensor) else feat for feat in player_features])
                kmeans = KMeans(n_clusters=2, random_state=0).fit(player_features_np)
                labels = kmeans.labels_
                confs = [boxes.data[idx][4].item() for idx in player_indices]

                team0 = [i for i, lbl in enumerate(labels) if lbl == 0]
                team1 = [i for i, lbl in enumerate(labels) if lbl == 1]

                top0 = sorted(team0, key=lambda i: confs[i], reverse=True)[:3]
                top1 = sorted(team1, key=lambda i: confs[i], reverse=True)[:3]
                
                selected = top0 + top1

                added0 = []
                if len(top0) < 3:
                    leftover1 = [i for i in team1 if i not in top1]
                    # confidence 낮은 순
                    added0 = sorted(leftover1, key=lambda i: confs[i])[:(3 - len(top0))]
                    selected += added0

                # 만약 top1이 3명 미만이면, team0의 leftover 중에서
                added1 = []
                if len(top1) < 3:
                    leftover0 = [i for i in team0 if i not in top0]
                    added1 = sorted(leftover0, key=lambda i: confs[i])[:(3 - len(top1))]
                    selected += added1
                ...
                selected_indices = [player_indices[i] for i in selected]
            ...
            with open(f"{self.txt_path}.txt", "w") as f:
                for j, box in enumerate(boxes.data):
                    ...
                    w = (x2 - x1) / img_w
                    h = (y2 - y1) / img_h
                    xywhn = [x_center.item(), y_center.item(), w.item(), h.item()]
